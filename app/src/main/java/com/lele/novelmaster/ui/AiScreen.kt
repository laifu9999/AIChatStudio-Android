...
fun AiScreen(nav: NavController) {
    ...
    AppScaffold(
        ...
        LazyColumn(Modifier.padding(pv).fillMaxSize(), contentPadding = PaddingValues(12.dp)) {
            ...
            // v6.9.73：智能分层注入开关——省 token 且不跑偏（默认开）
            item {
                val ctx = androidx.compose.ui.platform.LocalContext.current
                var leanChk by remember { mutableStateOf(InjectPrefs.leanInject(ctx)) }
                Spacer(Modifier.height(10.dp))
                ElevatedCard(Modifier.fillMaxWidth()) {
                    Row(
                        Modifier.padding(14.dp).fillMaxWidth(),
                        ...
                        Column(Modifier.weight(1f)) {
                            Text("🎯 智能分层注入（省 token）", style = MaterialTheme.typography.titleSmall)
                            Spacer(Modifier.height(2.dp))
                            Text(
                                "写章时：本章大纲/上一章结尾点名的人物、相关的世界观整卡注入；未登场人物与其余条目自动降为一行档案。人物多的书注入体积可省一半以上，且不随章节数膨胀。关闭=全部整卡注入（旧行为，最稳但最费）。",
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                        Switch(checked = leanChk, onCheckedChange = { on ->
                            leanChk = on
                            InjectPrefs.setLeanInject(ctx, on)
                        })
                    }
                }
            }
