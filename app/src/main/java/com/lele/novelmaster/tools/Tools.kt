...
object Tools {
    ...
    suspend fun foreshadowCheck(pid: Long): ToolResult {
        ...
        val (err, out) = WriterEngine.freeTask(
            ...
            "任务：伏笔体检（只分析，不修改任何卡与正文）。\n" +
                ...
                "\n【最近章节摘要】\n$summaryBlock\n\n" +
                "请输出：\n" +
                "1) 清单里每条未回收伏笔一行：先判真伪再判状态——真伪：「真伏笔（有明确悬念，读者在等某个答案）」/「疑似凑数（实为普通道具/人物/情节，无悬念可揭，多为旧版本误记）」；状态：「已过很久未回收（剧情已推进较远，读者可能遗忘）」/「仍在合理埋设期」/「摘要显示其实已回收但未标记」；\n" +
                "2) 疑似凑数的条目集中列出，明确建议：这些不该被回收，直接删除即可（在设定卡·伏笔钩子页点垃圾桶），以免占用注入名额、误导后续剧情；\n" +
                "3) 对确认真伏笔里需尽快回收的，给出建议的回收时机与方式（各一句话）；\n" +
                "4) 已回收伏笔若与最近摘要明显矛盾也要指出。\n" +
                "按紧急度从高到低排序，不要解释格式。"
        )
    ...
    private suspend fun cardsCheckInner(pid: Long): ToolResult {
        val (err, out) = cardsCheckOnce(pid)
    ...
    private suspend fun cardsApplyRepairInner(pid: Long): ToolResult {
        ...
        if (tailNote.isEmpty()) {
            tailNote = "\n\n⚠️ 已自动修复并复检 3 轮仍未收敛到 0 问题（剩余问题多需改大纲/正文才能根除，可跑「全书自检」）。可再说「设定体检」查看剩余项。"
        }
        val head = "🔧 已按体检报告确认修复（共 ${cards.size} 张卡）"
        // v6.9.59：未应用的方案逐条列出原因——不再让用户只能靠"再体检一次"盲验
        val skipBlock = if (allSkips.isEmpty()) "" else
        ...
        return try {
            ...
            val (err, out) = WriterEngine.freeTask(
                ...
                "任务：按作者要求修复设定卡。\n" +
                    ...
                    "5) 只输出格式行，不要任何其他解释。\n" +
                    "输出格式严格：\n" +
                    "【修复】每条一行：卡名｜修正后的完整卡片内容（全部需要修改的卡都输出，不设条数上限）\n" +
                    "【精简】每条一行：卡名｜去重后的完整卡片内容（不设条数上限，每卡60~200字，只留干货）\n" +
                    "【删除】每条一行：卡名（独有信息已并入其他卡、需要整卡删除的卡，不设张数上限）\n" +
                    "没有可改的就只输出【无修复项】。",
