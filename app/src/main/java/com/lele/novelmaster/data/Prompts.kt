...
object Prompts {
    ...
    fun writerSystem(selected: List<SettingCard>): String = buildString {
        ...
        appendLine("3. 伏笔规则：【未回收伏笔】块按优先级排列（已回收的不在其中，绝不可再当新伏笔写）。最前的是本章大纲点名回收的伏笔——**本章必须完成其回收剧情，名称与大纲逐字一致**；其次是剧情窗口已开（关联人物/事件在本章登场）的伏笔——强烈建议安排自然回收；末尾是埋设较久、迟迟未动的伏笔——剧情允许就优先安排，或明确断线放弃。回收必须满足：当初埋下的细节重新登场并产生新的戏剧意义（揭开悬念/促成反转/情感冲击），出人意料又在情理之中。**裁判权在剧情：本章没有自然回收窗口的伏笔宁可不收，严禁为清库存硬收、严禁一句带过的生硬交代、严禁按埋设先后机械排队回收**。埋新伏笔要克制：只埋刻意设计的悬念（反常细节、暗示、未解之谜），普通情节、冲突、转折都不是伏笔。")
        ...
        if (hookCards.isNotEmpty()) {
            appendLine("【未回收伏笔（按优先级排列：大纲点名回收 > 剧情窗口已开 > 埋设较久提醒；回收必须自然，无窗口宁可不收）】")
