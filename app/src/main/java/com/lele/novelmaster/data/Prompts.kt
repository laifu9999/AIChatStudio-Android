...
object Prompts {
    ...
    fun writerSystem(selected: List<SettingCard>): String = buildString {
        ...
        appendLine("3. 伏笔规则：【未回收伏笔】块列出当前埋设中的伏笔（已回收的不在其中，绝不可再当新伏笔写）。每章结合本章大纲判断：若某条伏笔的回收时机已到，就挑 1~2 条在剧情中自然回收——让当初埋下的细节重新登场并产生新的戏剧意义（揭开悬念/促成反转/情感冲击），回收要出人意料又在情理之中，严禁一句带过的生硬交代、严禁为凑数硬收；时机未到的继续自然推进。**若本章大纲明确写了「回收伏笔「名称」」，本章必须完成该伏笔的回收剧情，名称与大纲逐字一致**。埋新伏笔要克制：只埋刻意设计的悬念（反常细节、暗示、未解之谜），普通情节、冲突、转折都不是伏笔。")
    ...
    fun buildOutlineUser(
        ...
        append("请为第${from}章到第${to}章编写分章大纲，共${count}行。\n" +
            ...
            (if (openHooks.isNotEmpty())
                "\n伏笔统筹硬要求：①结合剧情节奏，为【未回收伏笔】里的伏笔安排回收——时机已到的，在该章大纲里明确写「回收伏笔「名称」：回收方式」（让当初埋下的细节重新登场并产生戏剧意义：揭开悬念/促成反转/情感冲击，出人意料又在情理之中，严禁一句带过的生硬交代）；早期埋设的优先回收，严禁只埋不收；②本批安排回收的伏笔不超过2条，宁可少而精；③本批不回收的伏笔若与某章剧情相关，在该章大纲写一笔「推进伏笔「名称」」保持线索不断；④严禁在大纲里新造与【未回收伏笔】重复的同类悬念。"
            else ""))
    }
