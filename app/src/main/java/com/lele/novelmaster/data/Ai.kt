...
object AiProviders {
    val presets = listOf(
        ...
        // v6.9.73：魔搭社区（ModelScope）API-Inference——OpenAI 兼容，注册即送每日免费额度，
        // 托管 Qwen/GLM/DeepSeek 等全家桶；「获取模型列表」按钮走 /models 可拉全部可调模型
        ProviderPreset(
            "魔搭社区 ModelScope（每日免费额度）", "openai", "https://api-inference.modelscope.cn/v1",
            "Qwen/Qwen3-32B", "访问令牌在 modelscope.cn 账号设置-访问令牌里创建；Qwen/GLM/DeepSeek 全系可调"
        ),
