    ...
    private suspend fun buildSystemText(pid: Long, continuing: Boolean, resumeMid: Boolean = false): String = buildString {
        ...
        appendLine(
            "【工具协议】需要执行操作时，在回复中输出工具块，格式必须是合法 JSON（两个字段缺一不可）：\n" +
                ...
                "— 一致性体检/修复：chapterSelfCheck{index}(单章自检·矛盾自动修正) | fullSelfCheck{}(全书逐章自检修复) | selfCheckProgress{}(自检进度) | undoSelfCheck{index}(撤销最近一次修改) | supplementChapter{index}(按大纲补写缺失剧情) | cardsCheck{}(设定体检·检查全部设定卡+分章大纲矛盾并自动修复) | cardsCheckReport{}(查看最近一次设定体检报告·只读)\n" +
