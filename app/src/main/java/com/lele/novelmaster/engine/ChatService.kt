    ...
    private suspend fun buildSystemText(pid: Long, continuing: Boolean, resumeMid: Boolean = false): String = buildString {
        ...
        appendLine(
            "【创作流程】\n" +
                ...
                "   **addCard 参数铁律（弱模型必读）**：参数名固定为 category/name/content——name=卡片名、content=卡内容，严禁写成 title/desc/简介 等其他键名（用错系统会报「名称和内容不能为空」）；保存顺序必须按 世界观→人物设定→主线剧情→核心冲突→支线任务→伏笔钩子→设定圣经→全书大纲 逐条输出工具块。**generateOutlines 永远是最后一步**：八类设定卡全部看到「已保存设定卡」回执之前调用会被系统直接拒绝，绝不能先建大纲后补设定。**一书一会话·源头绑定**：新会话里第一套设定确立本书后，本会话即从一而终绑定这本书——之后所有设定卡、分章大纲、正文都只为这本书创作，绝不在同一会话里再生成第二本书的设定卡（想要新书请作者在侧边栏新建会话）。\n" +
        ...
        appendLine(
            "【工具协议】需要执行操作时，在回复中输出工具块，格式必须是合法 JSON（两个字段缺一不可）：\n" +
                ...
                "— 一致性体检/修复：chapterSelfCheck{index}(单章自检·矛盾自动修正) | fullSelfCheck{}(全书逐章自检修复) | selfCheckProgress{}(自检进度) | undoSelfCheck{index}(撤销最近一次修改) | supplementChapter{index}(按大纲补写缺失剧情) | cardsCheck{}(设定体检·分批检查只出报告不改卡) | cardsCheckReport{}(查看最近一次体检报告·只读) | cardsApplyRepair{}(确认修复·按最近报告立即改卡·含删除重复卡瘦身) | cardsRepairWith{instruction}(按作者调整要求修复) | cardsSlim{}(设定瘦身·全部设定卡去重压缩抓重点) | setChapterOutline{index,text}(手动修改第N章大纲并同步镜像卡·text传新大纲全文)\n" +
                ...
                "【体检对话流程】cardsCheck 只出报告不改卡（分批进行有进度）：作者对报告有疑问 → 先 cardsCheckReport/readCard 查看再如实解释，可讨论可调整；作者确认要修 → 调 cardsApplyRepair 按报告立即修复（修复/精简/删除重复卡一并落卡）；作者带调整要求（如「保留李炎那张卡，把君既白并进去」）→ 调 cardsRepairWith{instruction} 按要求修复。没跑过体检时作者问体检相关，先跑 cardsCheck。作者抱怨卡太多太重复时，可建议「设定体检」+确认修复（重复卡会自动并入保留卡后删除瘦身）。\n" +
        ...
        if (pid > 0L) {
            ...
            if (project != null) {
                ...
                // v6.9.68：一书一会话写进系统提示词源头——不再只靠 addCard 事后拦截。
                // 弱模型收到新灵感就当新书全套执行，根源是它根本不知道本会话只能服务一本书
                appendLine(
                    "【一书一会话（最高优先级铁律）】本会话从一而终只服务《${project.title}》这一本书：所有 addCard/updateCard/generateOutlines/writeNextChapter 都只能为《${project.title}》工作。作者在会话里又给出新灵感、新题材时，先判断是否同属本书——同属《${project.title}》的 → 照常补设定、补分章大纲、写正文；明显是另一本书（另一套书名、另一套世界观或人物）→ 绝不为它输出任何 addCard/updateProject 工具块，只用一句话告知：本书《${project.title}》还在进行中，要开新书请在侧边栏新建会话。严禁在同一会话里生成第二本书的设定卡，严禁未经作者确认自动开新书；被拦下的内容不要换个分类或换个名字重试。"
                )
