    ...
    private suspend fun buildSystemText(pid: Long, continuing: Boolean, resumeMid: Boolean = false): String = buildString {
        ...
        appendLine(
            "【创作流程】\n" +
                ...
                "3.1) **系统硬性校验**：设定卡八类（世界观、人物设定、主线剧情、核心冲突、支线任务、伏笔钩子、设定圣经、全书大纲）+ 分章大纲 + 分卷大纲没建全时，writeNextChapter/startAutoWrite 会被系统拒绝并自动补全。所以必须先建全设定（工具回执可见）再写正文，严禁跳步、严禁没建完就调写作工具。\n" +
